...
router.route('/').get(
  async (req: express.Request, res: express.Response): Promise<void> => {
    try {
      ...
      res.status(200).json(users.map((user: IUser) => User.toResponse(user)));
    } catch (e) {
...
router.use('/:userId', userMiddleware);
...
router.route('/:userId').put(
  async (req: express.Request, res: express.Response): Promise<void> => {
    try {
      const user: IUser = await usersService.update(res.locals.user, req.body);
