...
const getTask = async (taskId: string, boardId: string): Promise<ITask> => {
  ...
  const task: ITask | undefined = tasks.find(
    (item: ITask): boolean => item.id === taskId
  );
...
const update = async (updatedTask: ITask): Promise<boolean> => {
  try {
    const tasks: ITask[] = (await getAll()).map(
      (cur: ITask): ITask => (cur.id === updatedTask.id ? updatedTask : cur)
    );
    await saveTasks(tasks);
...
const clearColumnIdByBoard = async (board: IBoard): Promise<void> => {
  ...
  const idList = columns.map(
    ({ id: columnId }: IBoardColumn): string | undefined => columnId
  );
